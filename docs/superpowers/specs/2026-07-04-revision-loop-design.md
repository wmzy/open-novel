...
**角色名来源**：前端从 `state.json.characters[].name` 读取角色列表，让用户从下拉框选择要传播的角色（不做自由输入——必须精确匹配现有角色名，否则扫描无意义）。

1. 前端将选定的角色名作为 `propagateCharacter` 传入。
2. 后端扫描 `.novel/chapters/*.md`，找包含 `propagateCharacter` 精确字符串的文件列表。
3. 对每个文件起**独立 revise run**（各自 runId、各自 baseSnapshot、各自 diff）。每个 run 的 `revisionNote` 在用户原始意见后追加 `（此章涉及角色：${propagateCharacter}）`。
4. 前端显示进度「修订 3/8 文件…」。
5. **启动前必须确认**：成本提示「这将在 N 个文件上各执行一次 agent 调用，预计 ~${N*20} 秒」（N = 命中文件数）。

**不合并成一次大 run**——一次 agent 调用改 8 个文件会导致注意力稀释、风格漂移。独立 run 各自聚焦，质量更高。

API：`POST /api/runs` body 增加 `propagateCharacter?: string`（角色全名）+ `propagateScope?: string[]`（可选限定范围，省略=全章节）。后端顺序执行（不并发，避免额度冲击），每完成一个 emit `propagate-progress` 事件 `{ completed: N, total: M }`。
...
- **重命名模式**：旧名从 `state.json.characters[].name` 下拉选择（不自由输入）；新名输入框失焦时调 `checkName` 预检，警告内联显示。
