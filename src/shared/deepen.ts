...
const CRITIQUE_PERSPECTIVES: Record<string, string[]> = {
  characters: [
    ...
    '跨阶段一致性审计师：你是跨阶段一致性审计师。**先读取 .novel/world/ 目录下相关卡片（如存在）**，审查角色设定是否符合世界观体系——角色的力量来源/社会身份/职业是否在世界规则内可解释？角色间关系结构是否与世界的社会阶层设定匹配？找出"角色与世界观脱节"的薄弱处。',
  ],
  world: [
  ...
  scenes: [
    ...
    '跨阶段一致性审计师：你是跨阶段一致性审计师。**先读取 .novel/outline/ 目录下相关章节卡片或 .novel/outline-brief.md（如存在）**，审查场景是否对齐大纲——每个场景是否推进了大纲中的事件？是否有大纲中重要事件在场景层缺失？找出"场景与大纲断裂"的薄弱处。',
  ],
  concept: [
    ...
    '跨阶段一致性审计师：你是跨阶段一致性审计师。**先读取 .novel/world/ 目录下相关卡片和 .novel/characters/profiles.md（如存在）**，审查概念是否在后续阶段有展开空间——核心冲突是否能在世界观中自然发生？概念的独特性是否已体现在角色和世界设定中？找出"概念与后续阶段脱节"的薄弱处。',
  ],
};
...
- **不要修改产出文件**（如 profiles.md / world/ 目录下卡片）——你只读不写
